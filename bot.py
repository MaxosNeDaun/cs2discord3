...
CHANNEL_ID = int(os.getenv("CHANNEL_ID", 1127290770571931739))  # ID канала для статуса сервера
...
class RegistrationView(discord.ui.View):
    ...
    def create_embed(self, registered_users):
        ...
        embed = discord.Embed(title="📝 Регистрация", description=self.description, color=discord.Color.blue())
...
async def get_server_info():
    try:
        ...
        player_list = "\n".join([p.name for p in players[:15]]) if players else "Пусто"

        embed = discord.Embed(title="Статус сервера CS2", color=discord.Color.green())
        embed.add_field(name="🟢 Онлайн", value=f"`{SERVER_IP[0]}:{SERVER_IP[1]}`", inline=False)
...
async def on_ready():
    init_db()
    # Регистрируем View, чтобы кнопки работали после перезагрузки
